...
{
    /* NMI has priority and is checked at instruction fetch time (before any instruction) */
    if (cpu->nmi_pending)
    {
        cpu_nmi(cpu);
        return 7;
    ...
    /* Check for IRQs after instruction completes (6502 checks at end of each instruction) */
    /* This is critical for instructions like CLI that modify the I flag */
    /* NMI is not checked here because it was already handled at the start */
    if (cpu->irq_pending && !get_flag_interrupt(cpu))
    {
        cpu_irq(cpu);
        return 7; /* IRQ takes 7 cycles and instruction doesn't complete normally */
    }
    ...
    if (cpu->page_crossed)
    {
        bool is_read_inst = (strcmp(inst->name, "LDA") == 0 ||
