...
bool c64_load_roms(C64 *c64, const char *rom_path)
{
    ...
    if (!c64_load_basic(c64, path)) success = false;
    
    snprintf(path, sizeof(path), "%s/kernal.rom", rom_path);
    if (!c64_load_kernal(c64, path)) success = false;
    
    snprintf(path, sizeof(path), "%s/char.rom", rom_path);
    if (!c64_load_char(c64, path)) success = false;
...
void c64_tick(C64 *c64)
{
    ...
    /* Update CPU interrupt lines from chips */
...
void c64_render_screen(C64 *c64)
{
    ...
    for (int i = 0; i < C64_SCREEN_COLS + 4; i++) printf("  ");
    printf("\033[0m\n");
