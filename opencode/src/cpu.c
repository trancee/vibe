...
void cpu_interrupt(C64 *sys, bool nmi)
{
    if (nmi || !cpu_get_flag(&sys->cpu, FLAG_I))
    {
        ...
        if (nmi)
        {
            sys->cpu.pc = mem_read(sys, NMI + 0) | (mem_read(sys, NMI + 1) << 8);
        }
        else
        {
            sys->cpu.pc = mem_read(sys, IRQ) | (mem_read(sys, 0xFFFF) << 8);
...
void cpu_step(C64 *sys)
{
    ...
    { // RTS
        ...
        sys->mem.cycles += 6; // RTS takes 6 cycles
        break;
    }
    ...
    { // Absolute,X
        ...
        if (sys->cpu.page_crossed)
            mem_read(sys, addr & 0xFF00); // Dummy read on page cross
        sys->cpu.a = cpu_adc_bcd(&sys->cpu, sys->cpu.a, data);
        break;
    }
    case 0x79:
    ...
    { // Absolute,X
        ...
        sys->mem.cycles += 4 + (sys->cpu.page_crossed ? 1 : 0); // Base + page cross
        break;
    }
    case 0xB9:
    { // Absolute,Y
