...
void STA(MOS6510 *cpu) {
    const opcode_t *op = &opcode_table[cpu->memory[mos6510_get_pc(cpu)]];
    uint16_t addr = get_operand_address(cpu, op->addr_mode);
...
void BIT(MOS6510 *cpu) {
    const opcode_t *op = &opcode_table[cpu->memory[mos6510_get_pc(cpu)]];
    uint8_t value = fetch_operand(cpu, op->addr_mode);
...
void ADC(MOS6510 *cpu) {
    const opcode_t *op = &opcode_table[cpu->memory[mos6510_get_pc(cpu)]];
    cpu->A = add_with_carry(cpu, cpu->A, fetch_operand(cpu, op->addr_mode));
    mos6510_set_pc(cpu, mos6510_get_pc(cpu) + 1);
    if (op->addr_mode != ADDR_IMP && op->addr_mode != ADDR_IMM) {
        mos6510_set_pc(cpu, mos6510_get_pc(cpu) + 1);
    }
...
void SBC(MOS6510 *cpu) {
    const opcode_t *op = &opcode_table[cpu->memory[mos6510_get_pc(cpu)]];
    cpu->A = subtract_with_borrow(cpu, cpu->A, fetch_operand(cpu, op->addr_mode));
    mos6510_set_pc(cpu, mos6510_get_pc(cpu) + 1);
    if (op->addr_mode != ADDR_IMP && op->addr_mode != ADDR_IMM) {
        mos6510_set_pc(cpu, mos6510_get_pc(cpu) + 1);
    }
...
void ASL(MOS6510 *cpu) {
    ...
    if (op->addr_mode == ADDR_IMP) {
        value = cpu->A;
        set_flag_carry(cpu, value & 0x80);
        value <<= 1;
        cpu->A = value;
    } else {
        addr = get_operand_address(cpu, op->addr_mode);
